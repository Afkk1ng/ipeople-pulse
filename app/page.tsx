...
import { BarChart3, Check, ClipboardCheck, Copy, Crown, Minus, Plus, ReceiptText, Settings2, Sparkles, Wrench } from 'lucide-react';
...
export default function Home() {
  ...
  return <main className="cyber-screen min-h-screen"><header className="cyber-header border-b backdrop-blur"><div className="mx-auto flex max-w-[1260px] items-center justify-between px-5 py-4"><div className="flex items-center gap-3"><div className="cyber-logo grid size-10 place-items-center rounded-xl text-xs font-black tracking-tighter">iP</div><div><p className="font-extrabold tracking-tight">iPeople <span className="cyber-pulse">PULSE</span></p><p className="text-xs text-[#7a8493]">Продажі · мотивація · команда</p></div></div><nav className="cyber-nav flex rounded-xl p-1"><button onClick={() => setView('calculator')} className={`rounded-lg px-3 py-2 text-sm font-bold ${view === 'calculator' ? 'bg-white text-[#246bfd] shadow-sm' : 'text-[#7a8493]'}`}>Калькулятор</button><button onClick={() => setView('leaders')} className={`rounded-lg px-3 py-2 text-sm font-bold ${view === 'leaders' ? 'bg-white text-[#246bfd] shadow-sm' : 'text-[#7a8493]'}`}>Лідери</button></nav></div></header>
  <div className="mx-auto max-w-[1260px] px-5 py-8"><div className="mb-7 flex flex-wrap items-end justify-between gap-3"><div><p className="mb-1 text-sm font-bold text-[#526fff]">{dayText}</p><h1 className="text-3xl font-extrabold tracking-tight">{view === 'calculator' ? 'Моя мотивація' : 'Таблиця лідерів'}</h1></div>{view === 'calculator' && <div className="cyber-signal" role="img" aria-label="iPeople PULSE активний"><span className="cyber-orbit"><i /></span><span className="cyber-bars"><i /><i /><i /><i /></span><span className="cyber-beam" /></div>}</div>
