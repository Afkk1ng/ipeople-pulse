...
import { BarChart3, Check, ClipboardCheck, Clock3, Crown, FileDown, Minus, Plus, ReceiptText, RotateCcw, Settings2, Sparkles, Wrench } from 'lucide-react';
...
export default function Home() {
  ...
  useEffect(() => { const cached = localStorage.getItem('motiva-reports'); if (cached) setReports(JSON.parse(cached)); setLoaded(true); fetch('/api/reports').then(response => response.ok ? response.json() : []).then((remote: DailyReport[]) => { if (Array.isArray(remote) && remote.length) setReports(remote); }).catch(() => {}); }, []);
  ...
  const telegramText = useMemo(() => {
    ...
    return [
      ...
      `🔧 Ремонти: ${money(repairProfit)} → <b>${money(calculations.repairsPay)}</b> (5%)`,
      `🎁 Бонуси: <b>${money(calculations.bonusPay)}</b>`, '', '━━━━━━━━━━━━━━',
      `💰 <b>В ЗП ЗА ЗМІНУ: ${money(calculations.total)}</b>`, '⚡ <i>iPeople PULSE · мотивація команди</i>',
    ].filter(Boolean).join('\n');
  }, [accessorySums, calculations, dayText, days, employee, planIndex, repairProfit, serviceAmount, serviceCount, serviceRate, techCount]);
  function createReport() { const date = new Date().toISOString().slice(0, 10); return { id: `${Date.now()}-${Math.random().toString(36).slice(2)}`, employee, date, base: calculations.base, tech: calculations.techPay, accessories: calculations.accessoriesPay, services: calculations.servicesPay, serviceUnits: serviceCount, repairs: calculations.repairsPay, bonuses: calculations.bonusPay, total: calculations.total, units: calculations.units, turnover: calculations.turnover }; }
  ...
  <div className="mx-auto max-w-[1260px] px-5 py-8"><div className="mb-7 flex flex-wrap items-end justify-between gap-3"><div><p className="mb-1 text-sm font-bold text-[#526fff]">{dayText}</p><h1 className="text-3xl font-extrabold tracking-tight">{view === 'calculator' ? 'Моя мотивація' : 'Таблиця лідерів'}</h1></div>{view === 'calculator' ? <button type="button" onClick={resetShift} className="cyber-reset" title="Скинути всі дані зміни" aria-label="Скинути всі дані зміни"><span className="reset-clock"><Clock3 /></span><RotateCcw className="reset-arrow" /></button> : <button type="button" onClick={resetLeaders} className="cyber-reset cyber-leaders-reset" title="Очистити таблицю лідерів" aria-label="Очистити таблицю лідерів"><span className="reset-clock"><Crown /></span><RotateCcw className="reset-arrow" /></button>}</div>
