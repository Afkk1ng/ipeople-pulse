...
import { BarChart3, Bell, BellOff, Check, ClipboardCheck, Clock3, Crown, FileDown, ListPlus, Minus, Plus, ReceiptText, RotateCcw, Settings2, Sparkles, Trash2, Wrench } from 'lucide-react';
...
type DailyReport = { id: string; employee: string; date: string; base: number; tech: number; accessories: number; services: number; serviceUnits: number; repairs: number; bonuses: number; total: number; units: number; turnover: number; approaches: number };
...
export default function Home() {
  ...
  const reportText = useMemo(() => {
    ...
    return [`iPeople PULSE · звіт зміни`, `Дата: ${dayText}`, `Співробітник: ${employee}`, '', `Зміна: ${days} дн. — ${money(calculations.base)}`, `Гаджети: ${calculations.units} шт. — ${money(calculations.techPay)}`, ...techRows, `Послуги: ${serviceCount} шт. · ${money(serviceAmount)} → ${money(calculations.servicesPay)} (${serviceRate}%)`, `Аксесуари: ${money(Object.values(accessorySums).reduce((sum, value) => sum + value, 0))} → ${money(calculations.accessoriesPay)}`, ...accessoryRows, `Ремонти: ${money(repairProfit)} → ${money(calculations.repairsPay)} (5%)`, `Підходи до клієнтів: ${approachCounts[employee] ?? 0}`, `Бонуси: ${money(calculations.bonusPay)}`, '', `В ЗП за зміну: ${money(calculations.total)}`].join('\n');
  }, [accessorySums, approachCounts, calculations, dayText, days, employee, planIndex, repairProfit, serviceAmount, serviceCount, serviceRate, techCount]);
  const telegramText = useMemo(() => {
    ...
    return [
      ...
      `🔧 Ремонти: ${money(repairProfit)} → <b>${money(calculations.repairsPay)}</b> (5%)`, `🎯 Підходи до клієнтів: <b>${approachCounts[employee] ?? 0}</b>`,
      `🎁 Бонуси: <b>${money(calculations.bonusPay)}</b>`, '', '━━━━━━━━━━━━━━',
      `💰 <b>В ЗП ЗА ЗМІНУ: ${money(calculations.total)}</b>`, '⚡ <i>iPeople PULSE · мотивація команди</i>',
    ].filter(Boolean).join('\n');
  }, [accessorySums, approachCounts, calculations, dayText, days, employee, planIndex, repairProfit, serviceAmount, serviceCount, serviceRate, techCount]);
  function createReport(): DailyReport { const date = new Date().toISOString().slice(0, 10); return { id: `${Date.now()}-${Math.random().toString(36).slice(2)}`, employee, date, base: calculations.base, tech: calculations.techPay, accessories: calculations.accessoriesPay, services: calculations.servicesPay, serviceUnits: serviceCount, repairs: calculations.repairsPay, bonuses: calculations.bonusPay, total: calculations.total, units: calculations.units, turnover: calculations.turnover, approaches: approachCounts[employee] ?? 0 }; }
