...
export async function GET() {
  ...
  try {
    ...
    const reports = result.results.filter(row => row.employee !== 'Технічна перевірка').map(row => {
      ...
      return { id: row.id, employee: row.employee, date: row.shift_date, base: number(details.base), tech: number(details.tech), accessories: number(details.accessories), services: number(details.services), serviceUnits: number(details.serviceUnits), repairs: number(details.repairs), focus: number(details.focus), bonuses: number(details.bonuses), total: row.total_pay, units: number(details.units), turnover: number(details.turnover), approaches: number(details.approaches) };
    });
    return json(reports);
